...
class SLSearcher(Displayable):
    ...
    def search(self,max_steps, prob_best=0, prob_anycon=1.0):
        ...
        prob_best is the probability that a best varaible (one in most conflict) is selected
        prob_anycon is the probability that a variabe in any conflict is selected
    ...
    def search_with_any_conflict(self, max_steps, prob_anycon=1.0):
        ...
        for i in range(max_steps):
            ...
            if len(self.csp.domains[var]) > 1:
                val = random_sample(set(self.csp.domains[var]) - set(self.current_assignment[var]))
...
from cspExamples import crossword1b

## Test Solving CSPs with Search:
#se1 = SLSearcher(crossword1b); print(se1.search(1000))
#print("Greedy Search")
#se2 = SLSearcher(crossword1b); print(se2.search(1000,1.0)) # greedy
SLSearcher.max_display_level=2  #more detailed display
print("Any Conflict")
se2 = SLSearcher(crossword1b); print(se2.search(1000, 0))  # any_conflict
#SLSearcher.max_display_level=1
#se2 = SLSearcher(crossword1b); print(se2.search(1000,0.7)) # 70% greedy; 30% any_conflict
